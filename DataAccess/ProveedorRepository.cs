...
namespace STOCKPAP.DataAccess
{
    public class ProveedorRepository
    {
        public List<Proveedor> ObtenerTodos()
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("SELECT Id, Empresa, Contacto, Telefono, Email, Direccion FROM Proveedores ORDER BY Empresa", conn))
        ...
        public bool AgregarProveedor(Proveedor p)
        {
            ...
            {
                conn.Open();
                using (var cmd = new NpgsqlCommand("INSERT INTO Proveedores (Empresa, Contacto, Telefono, Email, Direccion) VALUES (@e, @c, @t, @em, @d)", conn))
                {
